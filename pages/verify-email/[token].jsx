...
export async function getServerSideProps(ctx) {
  ...
  const { value: tokenDoc } = await ctx.req.db.collection('tokens').findOneAndDelete({ token, type: 'emailVerify' })

  if (!tokenDoc) {
    return { props: { success: false } }
  }

  await ctx.req.db.collection('users').updateOne({ _id: tokenDoc.userId }, { $set: { emailVerified: true } })
